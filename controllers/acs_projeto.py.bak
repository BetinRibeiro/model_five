...
def index():
    ...
    if pagina <= 0:
        redirect(URL(args=1))
    total = db((db.projeto.empresa==empresa.id)).count()
    ...
    registros = db((db.projeto.empresa==empresa.id)).select(
      limitby=limites,orderby=db.projeto.id
      )
    consul=(request.args(1))
    if (consul):
        registros = db((db.projeto.empresa==empresa.id)&((db.projeto.descricao.contains(consul)))).select(limitby=(0,paginacao))
...
def cadastrar():
    ...
    request.function='Cadastrar Projeto'
    usuario = db.usuario_empresa(db.usuario_empresa.usuario==auth.user.id)
...
def alterar():
    ...
    db.projeto.empresa.writable=False
    db.projeto.empresa.readable=False
    form = SQLFORM(db.projeto, projeto.id, deletable=False)
