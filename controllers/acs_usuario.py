...
def index():
    ...
    if pagina <= 0:
        redirect(URL(args=1))
    total = db((db.usuario_empresa.empresa==empresa.id)&(db.usuario_empresa.nome!="Betinho")).count()
    ...
    registros = db((db.usuario_empresa.empresa==empresa.id)&(db.usuario_empresa.nome!="Betinho")).select(
      limitby=limites,orderby=db.usuario_empresa.id
      )
    consul=(request.args(1))
    if (consul):
        registros = db((db.usuario_empresa.empresa==empresa.id)&((db.usuario_empresa.nome.contains(consul)))&(db.usuario_empresa.nome!="Betinho")).select(limitby=(0,paginacao))
